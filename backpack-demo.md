...
### Configure transformPresentation examples
...
```ts
{
    const pullRequest = new editModes.PullRequest(
        `apply-target-fingerprint-${Date.now()}`,
        `Apply fingerprint ${ci.parameters.fingerprint} to project`,
        "Nudge generated by Atomist",
        "",
...
```ts
{
    return new editModes.Commit("fingerprints", "merge all fingerprints to master until this branch is closed");
}    
```
