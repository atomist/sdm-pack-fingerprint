import { editModes, GitProject, GraphClient, guid, MappedParameter, MappedParameters, Parameter, Parameters } from "@atomist/automation-client";
...
function confirmUpdate(client: GraphClient): CodeTransform<ApplyTargetFingerprintParameters> {
    return async (p, cli) => {
        // await cli.addressChannels(`make an edit to the project in ${(p as GitProject).baseDir} to go to version ${cli.parameters.version}`);
        goals.applyFingerprint((p as GitProject).baseDir, queryPreferences(client), cli.parameters.fingerprint);
...
export function applyTargetFingerprint(client: GraphClient): CodeTransformRegistration<ApplyTargetFingerprintParameters> {
    return {
        ...
        transform: confirmUpdate(client),
    };
}
